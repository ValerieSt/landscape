...
for row in range(0, len(df1)):
    ...
    url = "https://www.google.com/search?q=%22%28ethics+OR+ethical+OR+governance%29+AROUND%2810%29%22+AND+%28AI+OR+artificial+intelligence%29+AND+%28principles+OR+guidelines+OR+recommendations%29+AND+%28%22" + input + "%22%29+AND+site%3A.gov" + input2
    print(url)
    df2.loc[len(df2)] = [url]
    count = count + 1
    #adding each search result to the table

df2.to_csv("output_Q5.csv")
print("Done.")
print("Your file (output_Q5.csv) is in your folder.")
